...
export class CommonService {
  ...
  decryptPassword(encryptedPassword) {
    try {
      ...
      // If decryption resulted in empty string, the password might be plain text
      if (!decrypted || '' === decrypted) {
        // Validate if it's a plain password string
        if (typeof encryptedPassword === 'string' && encryptedPassword.length >= 8) {
          return encryptedPassword;
        }
        throw new BadRequestException('Invalid Credentials');
      }

      const decryptedPassword = JSON.parse(decrypted);
      return decryptedPassword;
    } catch (error) {
      // If JSON parse fails or decryption fails, check if it's a valid plain password
      if (typeof encryptedPassword === 'string' && encryptedPassword.length >= 8) {
        // Return plain password as-is for direct API calls
        return encryptedPassword;
      }
      throw new BadRequestException('Invalid Credentials');
    }
